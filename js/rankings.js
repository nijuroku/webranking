class RankingManager {
    ...
    async loadRankingMain() {
        try {
            ...
            window.authManager.showNotification('Error al cargar el ranking main', 'error');
            this.hideLoading('main');
        }
    }
    ...
    async loadRankingExtra() {
        try {
            ...
            const evento = document.getElementById('eventoNombre').value || this.currentEvento;
            ...
            window.authManager.showNotification('Error al cargar el ranking extra', 'error');
            this.hideLoading('extra');
        }
    }
    ...
    async limpiarRankingExtra() {
        ...
        if (!confirm(`¿Estás seguro de eliminar todos los puntos del evento "${evento}"?`)) {
            return;
        }
        ...
        try {
            ...
            window.authManager.showNotification('Error al limpiar el ranking extra', 'error');
        }
    }
    ...
    exportRanking(tipo) {
        ...
        window.authManager.showNotification(`Ranking ${tipo} exportado correctamente`, 'success');
    }

    showLoading(tipo) {
        const tbody = tipo === 'main' ? 
